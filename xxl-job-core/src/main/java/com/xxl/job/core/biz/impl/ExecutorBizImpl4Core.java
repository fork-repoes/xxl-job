    ...
    public ReturnT<String> idleBeat(IdleBeatParam idleBeatParam) {
        ...
        if (isRunningOrHasQueue) {
            return new ReturnT<String>(
                    ReturnT.FAIL_CODE, "job thread is running or has trigger queue.");
        }
        return ReturnT.SUCCESS;
    }
        ...
        if (GlueTypeEnum.BEAN == glueTypeEnum) {
            ...
            if (jobHandler == null) {
                try {
                    ...
                    jobHandler =
                            new GlueJobHandler(originJobHandler, triggerParam.getGlueUpdatetime());
                } catch (Exception e) {
        ...
        if (jobThread == null) {
            jobThread =
                    XxlJobExecutor.registJobThread(
                            triggerParam.getJobId(), jobHandler, removeOldReason);
        }
    ...
    public ReturnT<LogResult> log(LogParam logParam) {
        ...
        String logFileName =
                XxlJobFileAppender.makeLogFileName(
                        new Date(logParam.getLogDateTim()), logParam.getLogId());
